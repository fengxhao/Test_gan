...
with tf.Session() as session:
    ...
    for i, device in enumerate(DEVICES_A):
        with tf.device(device):
            ...
            disc_real = tf.reshape(disc_all[:BATCH_SIZE/len(DEVICES_A)],[-1,1])
            disc_fake = tf.reshape(disc_all[BATCH_SIZE/len(DEVICES_A):],[-1,1])
            ...
            con_kernel_cost=0
            for i in range(10):
                find_index = tf.where(tf.equal(all_real_labels,i))
            ...
            disc_costs.append(-1*con_kernel_cost)
    ...
    for iteration in xrange(ITERS):
        ...
        for i in xrange(N_CRITIC):
            ...
            num_index=[]
            for ind in range(10):
                whlen = len(np.where(_labels==ind)[0])
                if whlen==0:
                    whlen=1
                num_index.append(whlen)
            num = np.shape(np.unique(_labels))[0]
            num_index.append(num)

            if CONDITIONAL and ACGAN:
                _disc_cost, _disc_wgan, _disc_acgan, _disc_acgan_acc, _disc_acgan_fake_acc, _ = session.run([disc_cost, disc_wgan, disc_acgan, disc_acgan_acc, disc_acgan_fake_acc, disc_train_op], feed_dict={all_real_data_int: _data, all_real_labels:_labels, _iteration:iteration,ind_t:num_index})
            else:
                _disc_cost, _ = session.run([disc_cost, disc_train_op], feed_dict={all_real_data_int: _data, all_real_labels:_labels, _iteration:iteration,ind_t:num_index})
