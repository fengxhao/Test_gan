...
gen_cost  = kernel_cost+0.1*con_kernel_cost+(class_loss_fake)
disc_cost = -1*(kernel_cost+0.1*con_kernel_cost)+(class_loss_real)+gp_cost+10*FSR_cost
...
def generate_image(frame, true_dist):
    ...
    lib.save_images.save_images(samples.reshape((128, 3, 32, 32)), './out_cifar_con0.1_fsr10/samples_{}.jpg'.format(frame))
...
with tf.Session() as session:
    ...
    for iteration in xrange(ITERS):
        ...
        lib.plot.plot('train disc cost_cifar_con0.1_fsr10', _disc_cost)
        lib.plot.plot('class_real_cifar_con0.1_fsr10',real)
        lib.plot.plot('class_fake_cifar_con0.1_fsr10',fake)
        lib.plot.plot('gp_cost_cifar_con0.1_fsr10',_gp)
        lib.plot.plot('con_kernel_cost_cifar_con0.1_fsr10',_con_cost)
