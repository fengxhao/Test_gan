...
gen_cost  = -tf.reduce_mean(disc_fake) + 1*(class_loss_fake)
disc_cost = tf.reduce_mean(disc_fake) - tf.reduce_mean(disc_real) + 1*(class_loss_fake+class_loss_real)+gp_cost


#gen_cost  = kernel_cost+1*(class_loss_fake)
#disc_cost = -1*(con_kernel_cost)+1*(class_loss_real+class_loss_fake)+gp_cost

gen_train_op = tf.train.AdamOptimizer(learning_rate=2e-4,beta1=0.5,beta2=0.9).minimize(gen_cost, var_list=gen_params)
disc_train_op = tf.train.AdamOptimizer(learning_rate=2e-4,beta1=0.5,beta2=0.9).minimize(disc_cost, var_list=disc_params)
...
fixed_noise_samples = Generator(100, label=fixed_labels,noise=fixed_noise)
...
def generate_image(frame, true_dist):
    ...
    lib.save_images.save_images(
        ...
        './save_cifar_image/samples_{}.jpg'.format(frame)
    )
