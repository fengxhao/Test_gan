...
def Generator(z,con,labels=None,reuse=False,nums=50):
    with tf.variable_scope("Generator") as scope:
        ...
        if z is None:
            ...
            con = tf.random_normal([nums,2])
        if labels is not None:
            z = tf.concat([z,labels,con],1)
...
def main(_):
    ...
    disc_cost = tf.reduce_mean(disc_fake) - tf.reduce_mean(disc_real) + 20*(class_loss_fake+class_loss_real+con_cost)
    ...
    if FLAGS.is_clip :
        ...
        source_logit,class_logit,_,_=Discriminator(interpolates,reuse=True)
    ...
    gen_save_image = Generator(tensor_noise,con=con_tensor,labels=label_tensor,reuse=True,nums=128)
    ...
    with tf.Session() as sess:
        ...
        for i in xrange(FLAGS.iter_range):
            ...
            if i%100==99:
                ...
                print gen_label_
                print "con_cost:"
                print sess.run(con_cost,feed_dict={X_image:data_x,y_label_index:data_y})
                print ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"
